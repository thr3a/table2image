...
function extractSegments(node: InlineNode, bold = false, italic = false): CellSegment[] {
  if (node.type === 'text') {
...
function parseTable(tableNode: any): { rows: Row[]; aligns: TableAlign[] } {
  const aligns: TableAlign[] = tableNode.align ?? [];
  const rows: Row[] = [];

  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  tableNode.children.forEach((rowNode: any, rowIdx: number) => {
    const isHeader = rowIdx === 0;
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const cells: Cell[] = rowNode.children.map((cellNode: any) => {
...
function measureSegmentsWidth(ctx: CanvasRenderingContext2D, segments: CellSegment[]): number {
  let w = 0;
...
function renderTable(rows: Row[], aligns: TableAlign[]): Buffer {
  ...
  for (let ri = 0; ri < rows.length; ri++) {
    ...
    for (let ci = 0; ci < colCount; ci++) {
      ...
      const bgColor = row.isHeader ? COLOR_HEADER_BG : ri % 2 === 0 ? COLOR_ROW_ODD : COLOR_ROW_EVEN;
...
async function main() {
  ...
  if (!inputFile) {
    console.error('Usage: npx tsx src/main2.ts <markdown-file>');
    process.exit(1);
  }
